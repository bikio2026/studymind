...
async function getTutorNotes(documentId) {
  const db = getClient()
  const result = await db.execute({ sql: 'SELECT notes FROM tutor_notes WHERE document_id = ?', args: [documentId] })
  return result.rows[0]?.notes || ''
}

async function saveTutorNotes(documentId, notes) {
  const db = getClient()
  await db.execute({
    sql: 'INSERT OR REPLACE INTO tutor_notes (document_id, notes, updated_at) VALUES (?, ?, ?)',
    args: [documentId, notes, Date.now()]
  })
}
...
async function handleAction(action, params) {
  switch (action) {
    ...
    case 'getTutorNotes': return getTutorNotes(params.documentId)
    case 'saveTutorNotes': return saveTutorNotes(params.documentId, params.notes)
